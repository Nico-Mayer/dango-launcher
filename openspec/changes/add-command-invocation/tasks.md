...
- [ ] 2.1 Define the command trait and the invocation context, and prove with a unit test that a built-in command can be invoked through it
- [ ] 2.2 Define the output sink carrying view trees and no-view outcomes, with a test that a command can push more than one tree
- [ ] 2.3 Add the host trait with the built-in native implementation, and test that a command declaring an unknown host is refused rather than run
- [ ] 2.4 Resolve a command by fully qualified identity, with a test that two extensions declaring the same unqualified identifier do not collide
- [ ] 2.5 Reject an invocation of a command whose extension is disabled, and test that the caller gets an unavailable result rather than a panic
- [ ] 2.6 Add the generation counter so a newer invocation supersedes an older one, with a test that stale output is dropped
- [ ] 2.7 Isolate a failing command so its error surfaces as a failure outcome and the launcher stays usable, with a test over a panicking command
...
- [ ] 3.4 Generalise `run_action` to dispatch by owning extension, and test that two extensions' results are each handled by their owner
...
- [ ] 4.1 Define the `SystemControl` trait covering lock, sleep, trash, running applications, and quit
- [ ] 4.2 Factor icon extraction into a platform helper both `AppIndexer` and `SystemControl` use, and confirm the applications index still shows icons

## 5. System extension: shared

- [ ] 5.1 Declare the manifest with its four commands, their modes, titles, keywords, and icons, and test that it validates
- [ ] 5.2 Implement lock and sleep as no-view commands over the trait, tested against a fake `SystemControl`
- [ ] 5.3 Implement empty trash as a view command pushing the confirmation detail view, tested for confirm, decline, and already-empty
- [ ] 5.4 Implement quit application as a view command pushing the running-application list with launcher-side filtering, tested against a fake
- [ ] 5.5 Exclude Dango from the running-application list, with a test
- [ ] 5.6 Report a failure when the chosen application has already exited, and treat a refusal to quit as success, both tested
...
- [ ] 6.4 List running applications with names and icons, verified against what the dock and the app switcher show
- [ ] 6.5 Ask an application to quit, verified against one that exits cleanly and one holding an unsaved document
